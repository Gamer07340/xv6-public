...
// For xv6, we use the system malloc/free directly without any wrapper
// This avoids conflicts with xv6's simple K&R allocator
...
static int tcc_compile(TCCState *s1)
{
    ...
    /* free defines here already, else they would be freed in tcc_close() */
    free_defines(NULL);
...
LIBTCCAPI TCCState *tcc_new(void)
{
    ...
    /* we add dummy defines for some common macros to avoid warnings */
    ...
    tcc_define_symbol(s, "__x86_64", NULL);
    tcc_define_symbol(s, "amd64", NULL);
#endif
...
# endif
    ...
    {
        char buf[32];
        sprintf(buf, "%d", GCC_MAJOR);
        tcc_define_symbol(s, "__GNUC__", buf);
        sprintf(buf, "%d", GCC_MINOR);
        tcc_define_symbol(s, "__GNUC_MINOR__", buf);
        // sprintf(buf, "%d", GCC_PATCHLEVEL); // GCC_PATCHLEVEL not defined in config.h
...
# endif
    ...
    /* For xv6: skip standard CRT files, use xv6 runtime instead */
...
LIBTCCAPI int tcc_add_file(TCCState *s, const char *filename)
{
    ...
    /* For xv6: always detect file type based on extension */
    const char *ext = filename + strlen(filename);
    while (ext > filename && *ext != '.' && *ext != '/') ext--;
    
    if (*ext == '.' && ext[1] == 'o' && ext[2] == '\0') {
        filetype = AFF_TYPE_BIN;
    } else if (*ext == '.' && ext[1] == 'S' && ext[2] == '\0') {
